...
namespace FlutterWave.Core.Tests.Unit.Services.Foundations.Subscriptions
{
    ...
    {
        [Fact]
        public async Task ShouldPostDeactivateSubscriptionByIdAsync()
        {
            // given 
            string inputSubscriptionId = GetRandomString();
            dynamic createRandomSubscriptionsProperties =
                     CreateRandomDeactivateSubscriptionsProperties();
            ...
            this.flutterWaveBrokerMock.Setup(broker =>
                broker.PostDeactivateSubscriptionAsync(inputSubscriptionId))
                    .ReturnsAsync(randomExternalSubscriptionResponse);

            // when
            Subscription actualSubscriptions =
               await this.subscriptionsService.PostDeactivateSubscriptionAsync(inputSubscriptionId);
