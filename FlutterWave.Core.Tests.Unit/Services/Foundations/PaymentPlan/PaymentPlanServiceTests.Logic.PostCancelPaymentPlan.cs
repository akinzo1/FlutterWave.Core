...
namespace FlutterWave.Core.Tests.Unit.Services.Foundations.PaymentPlans
{
    ...
    {
        [Fact]
        public async Task ShouldPostCancelPaymentPlanOfAccountAsync()
        {
            // given 
            ...
            this.flutterWaveBrokerMock.Setup(broker =>
                broker.PostCancelPaymentPlanAsync(inputPaymentPlanId))
                    .ReturnsAsync(randomExternalPaymentPlanResponse);

            // when
            PaymentPlan actualPaymentPlan =
               await this.paymentPlanService.PostCancelPaymentPlanAsync(inputPaymentPlanId);
