...
{
    internal partial class SettlementsService : ISettlementsService
    {
        ...
        public ValueTask<AllSettlements> GetAllSettlementsAsync() =>
        TryCatch(async () =>
        {
            ExternalSettlementsResponse externalSettlementsResponse = await flutterWaveBroker.GetAllSettlementsAsync();
            return ConvertToSettlementsResponse(externalSettlementsResponse);
        });

        public ValueTask<Settlement> GetSettlementsByIdAsync(string settlementId) =>
        TryCatch(async () =>
        {
            ValidateSettlementId(settlementId);
            ExternalSettlementResponse externalSettlementResponse = await flutterWaveBroker.GetSettlementByIdAsync(settlementId);
            return ConvertToSettlementResponse(externalSettlementResponse);
